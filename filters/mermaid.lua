  ...
  compile = function (self, code)
    ...
    return with_temporary_directory("diagram", function (tmpdir)
      return with_working_directory(tmpdir, function ()
        ...
        local puppeteer_config = 'puppeteer-config.json'
        local puppeteer_config_content = '{ "args": ["--no-sandbox"] }'
        write_file(puppeteer_config, puppeteer_config_content)

        write_file(infile, code)
        pipe(
          self.execpath or 'mmdc',
          {"--pdfFit", "--input", infile, "--output", outfile, "--puppeteerConfigFile", puppeteer_config},
          ''
        )
